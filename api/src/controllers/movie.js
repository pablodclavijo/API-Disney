...
async function getMoviesHandler (req, res){
    ...
    const movies = {}
    if(name) movies = await moviesDbInfo.searchMoviesByTitle(name, order ? order : null).catch(err => return status(500).send(err))
    if(genreId) movies = await moviesDbInfo.searchMoviesByGenre(genreId, order ? order : null).catch(err => return status(500).send(err))
...
async function getMovieByIdHandler (req, res){
    ...
    const {id} = req.params
    const movieDetail = await moviesDbInfo.getMovieDetail(id).catch(err => return status(500).send(err))
...
async function deleteMovieHandler (req, res) {
    ...
    const {id} = req.body
    const isDeleted = await moviesDbInfo.deleteMovie(id).catch(err => return status(500).send(err))
...
async function putMovieHandler (req, res) {
    ...
    const isUpdated = await moviesDbInfo.updateMovie(id, newValues).catch(err => return status(500).send(err))
...
async function postMovieHandler (req, res){
    ...
    const isPosted = await moviesDbInfo.createMovie(movieData).catch(err => return status(500).send(err))
