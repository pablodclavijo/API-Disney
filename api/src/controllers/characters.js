...
async function getCharactersHandler (req, res){
    ...
    characters = await charactersDbInfo.getAllCharacters().catch(err => {return res.status(500).send(err)})
...
async function getCharactersByIdHandler (req, res){
    ...
    const {id} = req.params
    const characterDetail = await charactersDbInfo.getCharacterDetail(id).catch(err => {return res.status(500).send(err)})
...
async function deleteCharacterHandler (req, res) {
    ...
    const {id} = req.body
    const isDeleted = await charactersDbInfo.deleteCharacter(id).catch(err => {return res.status(500).send(err)})
...
async function putCharacterHandler (req, res) {
    ...
    const isUpdated = await charactersDbInfo.updateCharacter(id, newValues).catch(err => { return res.status(500).send(err)})
...
async function postCharacterHandler (req, res){
    ...
    const isPosted = await charactersDbInfo.createCharacter(characterData).catch(err => {return res.status(500).send(err)})
